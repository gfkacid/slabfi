...
function parsePriceFromBody(body) {
  ...
  if (!Number.isFinite(n))
    throw new Error("Invalid plain-text price body");
  return BigInt(Math.trunc(n));
}
var fetchPriceUsd = (sendRequester, config) => {
...
var onCronTrigger = (runtime2) => {
  const priceUsd = new ClientCapability2().sendRequest(runtime2, fetchPriceUsd, consensusMedianAggregation())(runtime2.config).result();
  runtime2.log(`Consensus price (8 decimals): ${priceUsd.toString()}`);
  ...
  const collection = evmConfig.collection;
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: evmConfig.oracleConsumerAddress,
    report: report2
  }).result();
  ...
  runtime2.log(`OracleConsumer updated. tx: ${txHash}`);
